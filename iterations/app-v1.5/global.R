##Logan Lauton
##global.R - NBA Player HUD App v1.5
...
ptot <- read.csv("/cloud/project/NBA Player Stats(1950 - 2023).csv")
psal <- read.csv("~\\NBA Salaries(1990-2023).csv")
tpay <- read.csv("~\\NBA Payroll(1990-2023).csv")
pbox <- read.csv("/cloud/project/NBA Player Box Score Stats(1950 - 2023).csv")
...
ptot <- read.csv("/cloud/project/NBA Player Stats(2023).csv")

ptot <- subset(ptot, Tm != "TOT")

pbox <- read.csv("/cloud/project/NBA Player Box Score Stats(2023).csv")
...
ptot_actual <- read.csv("/cloud/project/NBA Player Stats(1950 - 2022).csv")

ptot <- merge(x = ptot_actual, y = ptot, all = TRUE)
...
pbox_actual <- read.csv("/cloud/project/NBA Player Box Score Stats(1950 - 2022).csv")
...
saveRDS(ptot, file = "/cloud/project/NBA_APP_v1.5_ptot_data.rds")
...
ptot_20 <- ptot[ptot$Season >= 2003 & ptot$Season <= 2023, ]
...
saveRDS(ptot_20, file = "/cloud/project/ptot_20_data.rds")
saveRDS(psal_20, "psal_20_data.rds")
saveRDS(tpay_20, "tpay_20_data.rds")
saveRDS(pbox, "/cloud/project/pbox_20_data.rds")

ptot <- readRDS("/cloud/project/NBA_APP_v1.5/app/ptot_data.rds")
...
as.Date(pbox$GAME_DATE,"%B %d, %Y")

pbox$date <- format(as.Date(pbox$GAME_DATE, format="%B %d, %Y"), "%m/%d/%Y")
pbox$date2 <- format(as.Date(pbox$GAME_DATE, format="%Y-%m-%d" ), "%m/%d/%Y")
pbox$date <- coalesce(pbox$date, pbox$date2)
pbox$GAME_DATE <- pbox$date
pbox <- pbox %>% select(-c(date2,date))

pbox$GAME_DATE <- as.Date(pbox$GAME_DATE, format="%m/%d/%Y")
pbox
######## MERGE COLUMNS
...
inj <- read.csv('/cloud/project/NBA Player Injury Stats(2003 - 2023).csv')
...
inj %>% filter(Acquired == "Dan Dickau")

inj[inj$Acquired == 'Dan Dickau', ]


trim.leading <- function (x)  sub("^\\s+", "", x)

inj$Acquired <- trim.leading(inj$Acquired)
inj$Relinquished <- trim.leading(inj$Relinquished)

inj$Season <- format(as.Date(inj$Date, format="%Y-%m-%d"),"%Y")

inj$Season <- as.numeric(inj$Season)
saveRDS(inj,"/cloud/project/inj_20_data.rds")



plt <-
  dplyr::filter(ptot, Player == "Victor Oladipo") %>%
  group_by(Player, Season) %>%
  summarise(
    BLK = sum(BLK * G) / sum(G),
    AST = sum(AST * G) / sum(G),
...
inj_dat<-
  dplyr::filter(inj, Relinquished == "Victor Oladipo")
